...
#define MAVLINK_MSG_ID_LOCATION_REDUCED 4
...
#define MAVLINK_MSG_ID_LOCATION_REDUCED_CRC 120
#define MAVLINK_MSG_ID_4_CRC 120
...
#define MAVLINK_MESSAGE_INFO_LOCATION_REDUCED { \
    ...
    "LOCATION_REDUCED", \
    3, \
    {  { "uwb_address", NULL, MAVLINK_TYPE_UINT16_T, 0, 8, offsetof(mavlink_location_reduced_t, uwb_address) }, \
...
{
    return mavlink_msg_location_reduced_pack(system_id, component_id, msg, location_reduced->uwb_address, location_reduced->location_x, location_reduced->location_y);
}
...
{
    return mavlink_msg_location_reduced_pack_chan(system_id, component_id, chan, msg, location_reduced->uwb_address, location_reduced->location_x, location_reduced->location_y);
}
...
static inline void mavlink_msg_location_reduced_send(mavlink_channel_t chan, uint16_t uwb_address, float location_x, float location_y)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_LOCATION_REDUCED_LEN];
    _mav_put_float(buf, 0, location_x);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_location_reduced_send(chan, location_reduced->uwb_address, location_reduced->location_x, location_reduced->location_y);
#else
 ...
 */
static inline void mavlink_msg_location_reduced_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint16_t uwb_address, float location_x, float location_y)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
/**
 * @brief Get field uwb_address from location_reduced message
 *
 * @return  
 */
static inline uint16_t mavlink_msg_location_reduced_get_uwb_address(const mavlink_message_t* msg)
{
...
static inline void mavlink_msg_location_reduced_decode(const mavlink_message_t* msg, mavlink_location_reduced_t* location_reduced)
{
    ...
    location_reduced->uwb_address = mavlink_msg_location_reduced_get_uwb_address(msg);
#else
