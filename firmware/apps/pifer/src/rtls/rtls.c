    ...
    while(1){
        ...
        for(int j=0; j<ANCHOR_NUM; j++){
            if(g_distance.anchors[j] != 0 && g_distance.updated[j]){
                ...
                if(node != NULL){
                    mavlink_msg_tof_pack(0,0,&mavlink_msg, node->addr, node->location_x, node->location_y, node->location_z, g_distance.tofs[j]);
                    uint16_t len = mavlink_msg_to_send_buffer((uint8_t*)mav_send_buf, &mavlink_msg);
                    serial_write(mav_send_buf, len);
                    // printf("0x%04X: %04d %04d %04d %04ld\n", g_distance.anchors[j], (int)(node->location_x*1000), (int)(node->location_y*1000), (int)(node->location_z*1000), g_distance.tofs[j]);
                }
            }
        }
    }
