...
All TDMA packets include two parts: IEEE frame header and payload:
\begin{itemize}
    \item IEEE fame header conforms the 802.15.4-2015 standard
    \item Payload field may contain more than one message. Each message have TLV (type-length-value) format as shown in figure \ref{fig:tdma_frame_payload}.
\end{itemize}
...
\subsubsection{Frame Control}
\subsubsection{Sequenct Number}
\subsubsection{Pan ID}
\subsubsection{Dst Address}
\subsubsection{Src Address}
\subsubsection{Payload}

TLV format, the Type-Length-Value encoding, is used for data exchanging between nodes. Data in TLV format always begins with the type byte, followed by the length byte, and then followed by a variable number of value bytes [0 to 253] as specified by the length byte.
