...
This section provides an overview of the ranging network operation and layered system architecture.

\section{Prerequisite}
Consider an UWB system which consist of a MCU and a UWB radio IC.
Define:
\begin{itemize}
    \item Host: the MCU 
...
\begin{itemize}
    ...
    \item \textbf{Relay}: Relay nodes repeat the clock calibration packet to extend the network physical area. Before repeating the packets, they must do some modification so the receiver can sync with the  master node. There is a constraint in the maximum of repeat level which will be analyzed afterward.
\end{itemize}
...
There are two timeming 
Timestamp:
\begin{itemize}
    \item $e_m$ Master epoch: Device transmission timestamp
    \item $e_l$ Local epoch: Device receive timestamp
    \item $e_o$ Os epoch: Host receive timestamp
\end{itemize}
